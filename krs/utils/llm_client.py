...
class KrsGPTClient:
    ...
    def __init__(self, reinitialize=False, reset_history=False, device='cpu'):
        ...
        self.history = [] if reset_history else self.history
    ...
    def init_huggingface_client(self, reinitialize=False):
        ...
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(self.model)
            self.model_hf = AutoModelForCausalLM.from_pretrained(self.model)
            self.pipeline = pipeline('text-generation', model=self.model_hf, tokenizer=self.tokenizer, device=0 if self.device == 'cuda' else -1)
